...
/**
 * @author SNIPPIK
 * @description Middleware для проверки подключения к голосовому каналу клиента
 * @usage Для команд, где требуется голосовой канал клиента
 * @class ClientVoiceChannel
 * @extends Assign
 */
class ClientVoiceChannel extends Assign<middleware<CommandInteraction>> {
    public constructor() {
        super({
            name: "client_voice",
            callback: (ctx) => {
                const VoiceChannel = db.voice.get(ctx.guildId);
 ...
 */
export default [VoiceChannel, OtherVoiceChannel, ClientVoiceChannel];