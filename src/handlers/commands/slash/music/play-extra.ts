...
@Options({
    ...
    request: createStringOption({
        ...
        autocomplete: (ctx) => {
            if (!ctx.options.hoistedOptions[1].value || ctx.options.hoistedOptions[1].value === "") {
                return ctx.respond([
                    {
            ...
            return db.commands.playAutocomplete(ctx, platform, ctx.options.hoistedOptions[1].value as string);
        }
    })
})
@Locales({
