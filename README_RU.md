...
> Но голосовой системе просто не дозволено терять аудио пакеты даже при критической нагрузке, при блокировках UDP пакеты будут возвращены в очередь для повторной отправки!
---
...
- Есть доступ к изменению режима декодера (`voip`, `audio`, `lowdelay`), вкл/выкл `VBR`, вкл/выкл `packet lost` на этапе загрузки, так-же есть `FEC`
- Присутствуют строгие ограничения задержек для устранения поломки аудио, их можно так-же изменить!
...
- Основная логика обработки голоса вынесена в нативный модуль на Rust (src-rs), что гарантирует стабильность даже при высоком event loop lag в Node.js.
  ...
  - **FFmpeg Integration**: Используется для гибкого декодирования аудио и применения сложных фильтров. Порядок включения фильтров очень важен!!!
...
> [!WARNING]
> Потери на стороне клиента невозможны. Если потеря произошла случайно, вы увидите это в сообщении «текущий трек».  
> Если ваше интернет-соединение нестабильно, потери будут происходить в любом случае.  
> Полностью исключить «потерю пакетов» невозможно из-за протокола UDP и других ограничений Discord.
...
│  • WatKLOK получает не полую обратную связь                                         │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
</details>
