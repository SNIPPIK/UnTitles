...
> Полностью обойти `packet lost` не возможно, из-за протокола `UDP` и прочих ограничений `discord`  
...
- Полностью `fallback` система, нет трека на 1 платформе найдется на другой! Ошибки не проблема!
